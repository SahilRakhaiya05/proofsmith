...
export const agents: AgentDefinition[] = [
  ...
  },
  {
    ...
    name: "Maker Agent",
    role: "maker",
    summary: "Implements the smallest correction via Google Gemini + optional E2B worktree sandbox.",
    inputs: ["contract", "failing evidence", "repo snapshot"],
    outputs: ["plan", "branch commits", "PR draft"],
    tools: ["Google Gemini", "E2B sandbox", "git", "/api/ai/agent"],
    canApproveMerge: false,
    status: "ready",
  },
  {
    id: "agent_gemini_orchestrator",
    name: "Gemini Orchestrator",
    role: "maker",
    summary: "Routes triage → maker → reviewer prompts through the best available Gemini model from the live API.",
    inputs: ["issue", "failure bundle", "live APP_URL"],
    outputs: ["agent transcript", "loop iteration line"],
    tools: ["Google Gemini listModels", "generateContent", "/api/loop/iterate"],
    canApproveMerge: false,
    status: "ready",
  },
  {
  ...
  },
  {
    ...
    name: "Reviewer Agent",
    role: "reviewer",
    summary: "Independent code review against contract, security policy, and diff budget via Gemini.",
    inputs: ["PR diff", "contract"],
    outputs: ["review verdict", "blocking comments"],
    tools: ["GitHub PR API", "Google Gemini", "/api/ai/agent"],
    canApproveMerge: false,
    status: "ready",
  },
  {
